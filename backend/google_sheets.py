        ...
        try:
            ...
            for record in records:
                ...
                processed_data.append({
                    ...
                    'parking_spot_id': record.get('slot_id') or record.get('Device_ID') or None,
                    'zone': record.get('Zone') or record.get('zone') or None,
                    'vehicle_detected': record.get('Vehicle_Detected') or record.get('vehicle_detected') or None,
                    'parking_duration': record.get('parking_duration') or None,
                    'raw': record
                })

            # Keep only the latest record per device_id (deduplicated)
            latest_by_device = {}
            for row in processed_data:
                key = row.get('device_id') or row.get('location') or row.get('timestamp')
                latest_by_device[key] = row

            # Filter to only devices that have sent data within the last 7 days
