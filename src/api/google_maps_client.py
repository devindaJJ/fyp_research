...
class GoogleMapsClient:
    ...
    def __init__(self, api_key: str):
        # Import here so tests can run without the googlemaps package installed
        try:
            import googlemaps
            self.client = googlemaps.Client(key=api_key)
        except ImportError:
            self.client = None
            logger.warning("googlemaps library not installed; Google API calls will fail if used.")
